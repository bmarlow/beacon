			...
			map[string]interface{}{
				...
				"rules": []interface{}{
					rule("BeaconGatewayVIPWithdrawn", "beacon_gateway_advertised == 0", "2m", "warning",
						...
						"Beacon has withdrawn the MetalLB advertisement for Gateway {{ $labels.gateway_namespace }}/{{ $labels.gateway_name }} on cluster {{ $labels.cluster }} for at least 2 minutes."),
					rule("BeaconGatewayUnhealthy", "beacon_gateway_healthy == 0", "5m", "warning",
						"Beacon Gateway is unhealthy",
						"Gateway {{ $labels.gateway_namespace }}/{{ $labels.gateway_name }} on cluster {{ $labels.cluster }} has been unhealthy for at least 5 minutes."),
					rule("BeaconManyWithdrawnIPs",
						...
						"A large-scale backend outage may be in progress on cluster {{ $labels.cluster }}."),
					rule("BeaconReconcileErrors", "rate(beacon_reconcile_errors_total[5m]) > 0", "15m", "warning",
						"Beacon is experiencing reconcile errors",
						"The Beacon controller on cluster {{ $labels.cluster }} has been logging reconcile errors for at least 15 minutes."),
